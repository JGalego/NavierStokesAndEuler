...
export const proofStages: ProofStage[] = [
  ...
  },
  {
    ...
    range: [1, 3],
    summary: 'Compact support supplies the decay condition; the factor $\\nu^2$ supplies the right scaling.',
    story:
      'Now fix $\\nu > 0$. The initial velocity is still zero, but the force becomes $\\nu^2 f$. Since $f$ is smooth and compactly supported in space, the rescaled force satisfies Comparator’s weighted decay condition.',
    bridge:
  ...
  },
  {
    ...
    range: [4, 22],
    summary: 'A supposed global solution at viscosity $\\nu$ can be rescaled to viscosity one, where the singular candidate is waiting.',
    story:
      'Assume, for contradiction, that $v$ and $q$ solve the problem globally. We define $v_1$ and $q_1$ by rescaling space, time, and amplitude with powers of $\\nu^{-1}$. Most of this stage is bookkeeping: proving that the new fields are smooth and divergence-free, satisfy the normalized equation, remain in $L^2$, and carry a uniform energy bound.',
    bridge:
      'The scaling puts $v_1$ and the candidate $u$ under the same viscosity-one equation and the same force. The uniqueness theorem now applies to both.',
  ...
  },
  {
    ...
    story:
      'At $t = 0$ the two velocities agree because both initial data are zero. For $t > 0$, we put both solutions on the closed slab $[0,t]$. The singular time is still ahead: $u$ is smooth and compactly supported there, and $v_1$ inherits a global energy bound. Those are the hypotheses needed for whole-space uniqueness.',
    bridge:
  ...
  },
  {
    ...
    range: [43, 45],
    summary: 'Uniqueness forces $u = v_1$ before time one. The candidate theorem says that no globally smooth $v_1$ can do this.',
    story:
      'Take any $t < 1$. Finite-energy uniqueness identifies $u$ and $v_1$ throughout $[0,t] \\times \\mathbb{R}^3$. Repeating the argument for every such $t$ gives exactly the pointwise agreement ruled out by $\\mathtt{h.not\\_global\\_agreement}$. There is the contradiction.',
    bridge:
      'apply $\\mathtt{h.not\\_global\\_agreement}$ turns the target False into the missing agreement statement. The final exact supplies it from uniqueness.',
    ...
    answer: 'apply $\\mathtt{h.not\\_global\\_agreement}$ sets the trap. The final exact supplies the agreement it forbids, and the outer application closes False.',
  },
]
...
const exactLessons: Record<number, StepLesson> = {
  ...
  2: {
    ...
    detail: 'Lean infers the type of $\\mathtt{hForceDecay}$ from the result type of $\\mathtt{forceConditionDecay}$.',
    concept: 'local lemma',
  },
  3: {
    label: 'Commit to the data',
    summary: 'The witnesses are zero initial velocity and $\\nu^2 f$. refine writes them down and leaves non-existence as the goal.',
    detail: 'Each ?_ passed to refine becomes a new goal; the supplied tuple fields disappear.',
    concept: 'refinement',
  },
  4: {
    label: 'Assume a global solution',
    summary: 'Suppose $v$ and $q$ solve the problem globally, and name the accompanying proof $\\mathtt{hGlobal}$.',
  ...
  5: {
    ...
    summary: 'Define $v_1$ by rescaling space, time, and velocity amplitude with $\\nu^{-1}$.',
    detail: 'let gives the expression a local name, so later goals can refer to $v_1$.',
    concept: 'local definition',
  },
  6: {
    label: 'Name the scaled pressure',
    summary: 'Define pressure with the matching $\\nu^{-2}$ amplitude and $\\nu^{-1}$ spacetime scale.',
  ...
  7: {
    ...
    summary: 'Prove locally that $v_1$ and $q_1$ form a viscosity-one global solution.',
  ...
  11: {
    ...
    summary: 'Provide smoothness first. Lean displays the five fields of $\\mathtt{GlobalSolutionRn}$ that remain.',
    detail: 'The goals cover initial data, incompressibility, the equation, spatial $L^2$ integrability, and uniform energy.',
  ...
  15: {
    ...
    detail: 'Each term picks up the appropriate power of $\\nu$, and the factors cancel.',
    concept: 'scaling covariance',
  },
  18: {
    label: 'Take the energy bound',
    summary: 'Take an energy bound $E$ and proof $\\mathtt{hE}$ from the assumed global solution.',
    detail: 'The existential is now represented by $E$ and $\\mathtt{hE}$ in the local context.',
    concept: 'witness extraction',
  },
  22: {
    label: 'Scale the energy estimate',
    summary: 'Multiply the original strict bound by the positive factor $\\nu^{-2}$.',
  ...
  23: {
    ...
    detail: 'apply $\\mathtt{h.not\\_global\\_agreement}$ replaces the contradiction with the pointwise agreement expected by that theorem.',
    concept: 'backward reasoning',
  },
  26: {
    label: 'Separate $t = 0$',
    summary: 'Prove the $t = 0$ case from initial data; send $t > 0$ to uniqueness.',
    detail: 'by_cases supplies $t = 0$ and $t \\ne 0$ as hypotheses in separate goals.',
    concept: 'case split',
  },
  30: {
    label: 'Stay below time one',
    summary: 'Show that the comparison slab $[0,t]$ lies before the singular time.',
    detail: 'The bound $t < 1$ puts every point of the closed slab in $\\mathtt{preSingularDomain}$.',
  ...
  36: {
    ...
    detail: 'Because $\\operatorname{tsupport}$ is a closure, $\\mathtt{closure\\_minimal}$ needs ordinary support containment and closedness of $K$.',
  ...
  43: {
    ...
    summary: 'Apply the whole-space finite-energy uniqueness theorem to $u$ and $v_1$ on $[0,t]$.',
  ...
  45: {
    ...
    detail: 'exact fills the equality requested by $\\mathtt{h.not\\_global\\_agreement}$, so the proof ends.',
...
const inferredLessons: Array<{
...
}> = [
  {
    ...
    lesson: {
      ...
      summary: 'The theorem returns data hidden behind $\\exists$. obtain gives those values names.',
